...
def post_processing(mask, frame):
    ...
    # Smooth Borders
    mask_blurred = cv2.GaussianBlur(colored_mask, (121, 121), 0)  
    _, mask_blurred = cv2.threshold(mask_blurred, 100, 255, cv2.THRESH_BINARY)

    # Blend with frame
    output = cv2.addWeighted(frame, 1.0, mask_blurred, 0.5, 0)
